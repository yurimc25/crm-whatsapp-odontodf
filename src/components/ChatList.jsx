...
import { formatTimeSince } from "../services/waha";

export default function ChatList({ chats, activeId, search, onSearch, onSelect, loading }) {
  // Ordena: sem resposta há mais tempo primeiro, depois por lastTime
  const sorted = useMemo(() => {
    return [...chats].sort((a, b) => {
      ...
      // Quem não foi respondido há mais tempo vem primeiro
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"100%", overflow:"hidden" }}>
      <div style={{ padding:"10px 12px", borderBottom:"1px solid #1a2e22" }}>
        <input value={search} onChange={e => onSearch(e.target.value)}
          ...
          style={{ width:"100%", background:"#111a15", border:"1px solid #1e3028",
...
function ChatItem({ chat, active, onClick }) {
  ...
  const timeSince = formatTimeSince(chat.lastPatientTs);

  // Urgência por tempo sem resposta
  const urgColor = !timeSince ? "#3a7055"
    : chat.lastPatientTs && (Date.now() - new Date(chat.lastPatientTs).getTime()) > 4 * 3600000
      ? "#c0412c"   // > 4h = vermelho
      : chat.lastPatientTs && (Date.now() - new Date(chat.lastPatientTs).getTime()) > 1 * 3600000
        ? "#b56a00"  // > 1h = amarelo
        : "#0d7d62"; // < 1h = verde
  ...
  return (
    <div onClick={onClick} draggable
      ...
      <div style={{ flex:1, minWidth:0 }}>
        {/* Linha 1: nome + tempo sem resposta */}
        ...
        {/* Linha 2: número */}
        <div style={{ color:"#2a5040", fontSize:11,
          fontFamily: info.hasContact ? "inherit" : "'DM Mono', monospace",
          overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", marginBottom:3 }}>
          {info.phone}
        </div>

        {/* Linha 3: última mensagem real */}
        <div style={{ color:"#3a5244", fontSize:11,
        ...
        <div style={{ display:"flex", gap:4, alignItems:"center", flexWrap:"wrap" }}>
          {(chat.tags || []).map(tag => {
            ...
            return (
              <span key={tag} style={{ background:tc.bg, color:tc.text,
                fontSize:9, fontWeight:700, padding:"1px 6px",
                borderRadius:4, textTransform:"uppercase", letterSpacing:.5 }}>{tag}</span>
            );
          })}
          {chat.unread > 0 && (
            <span style={{ marginLeft:"auto", background:"#0d7d62", color:"#fff",
