...
export function normalizeMessage(wahaMsg) {
  ...
  const hasMedia = ["image","video","audio","voice","document","sticker"].includes(type);
  const media    = hasMedia ? {
    type,
    // URL direta se já vier embutida
    url:      wahaMsg.media?.url || wahaMsg._data?.mediaUrl || null,
    // mimetype para decidir como renderizar
    mimetype: wahaMsg.media?.mimetype || wahaMsg._data?.mimetype || null,
    filename: wahaMsg.media?.filename || wahaMsg._data?.filename || null,
    // ID da mensagem para fazer download via proxy
    msgId:    wahaMsg.id || null,
  } : null;
